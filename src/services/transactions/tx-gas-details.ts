...
export const setGasDetailsForTransaction = (
  ...
  if (gasDetails.evmGasType !== evmGasType) {
    ...
    throw new Error(
      `Invalid evmGasType for ${networkName} (${transactionType}): expected Type${evmGasType}, received Type${gasDetails.evmGasType} in gasDetails. Retrieve appropriate gas type with getEVMGasTypeForTransaction (dop-sharedmodels).`,
    );
  }
